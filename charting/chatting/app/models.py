...
class Employee(models.Model):
    ...
    profile_image = models.ImageField(
        upload_to='profile_images/', null=True, blank=True, default=None
    )
    about = models.CharField(
        max_length=500, blank=True, default="Hey there! I'm using Chat App"
    )
    ...
    def get_avatar_url(self):
        ...
        return (
            f"https://ui-avatars.com/api/?name={self.name.replace(' ', '+')}"
            f"&background=random&size=200"
        )
...
class ChatGroup(models.Model):
    ...
    members = models.ManyToManyField(
        Employee, related_name="group_memberships", blank=True
    )
    ...
    group_image = models.ImageField(
        upload_to='group_images/', null=True, blank=True
    )
    ...
    def get_group_image_url(self):
        ...
        return (
            f"https://ui-avatars.com/api/?name={self.name.replace(' ', '+')}"
            f"&background=00a884&color=fff&size=200"
        )
...
class Message(models.Model):
    ...
    file = models.FileField(
        upload_to='message_files/%Y/%m/%d/', null=True, blank=True
    )
    ...
    starred_by = models.ManyToManyField(
        Employee, related_name='starred_messages', blank=True
    )
...
class MessageDeletion(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="deleted_messages"
    )
...
class MessageReaction(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="message_reactions"
    )
...
class SavedMeetLink(models.Model):
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="saved_meets"
    )
...
class MeetingInvitation(models.Model):
    ...
    message = models.ForeignKey(
        Message, on_delete=models.CASCADE, related_name="invitations"
    )
    invitee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="meeting_invitations"
    )
...
class AdminActivityLog(models.Model):
    ACTION_CHOICES = [
        ...
        ('change_chat_permission', 'Changed Group Chat Permission'),
    ]
    admin = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="admin_activities"
    )
    action = models.CharField(max_length=50, choices=ACTION_CHOICES)
    target_employee = models.ForeignKey(
        Employee, on_delete=models.SET_NULL,
        null=True, blank=True, related_name="admin_views"
    )
